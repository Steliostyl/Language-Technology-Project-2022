...
def train_Classifier(d_categories, tf_idf_vector):
    ...
    model = MultinomialNB().fit(tf_idf_vector, d_categories)
    return model

def predict_Category(documents, classifier):
...
# Predict category of a random document from our dataset
twenty_news_test = fetch_20newsgroups(subset='test', shuffle=True, random_state=21)
predict_Category(twenty_news_test.data, classifier)
